...
export const usePurchaseVoucher = () => {
  ...
  const resetLineFields = () => {
    form.reset({
      purchaseType: form.getValues("purchaseType") || "N",
      orderPurchaseType: form.getValues("orderPurchaseType") || "R",
      purchaseDate: undefined,
      partyId: "",
      purchaseNo: "",
      itemId: "",
      quantity: "",
      rate: "",
      roundOff: form.getValues("roundOff") || "",
      discount: form.getValues("discount") || "",
      transMode: form.getValues("transMode") || "C",
      bankId: form.getValues("bankId") || "",
    });
  ...
  const handleAddPurchase = () => {
    if (orgId && purchaseTableData.length > 0) addPurchaseVoucherApiCall(orgId);
  };
  ...
  const addPurchaseVoucherApiCall = async (
    // item: PurchaseVoucherFormData,
    orgId: number
  ) => {
    ...
    const data = {
      org_id: orgId,
      pur_date: form.getValues("purchaseDate")
        ? format(form.getValues("purchaseDate"), "yyyy-MM-dd")
        : "",
      pur_no: form.getValues("purchaseNo"),
      party_id: form.getValues("partyId"),
      tot_amount: purchaseTableData.reduce((acc, item) => {
        const rate = item.taxableTotal ? item.taxableTotal : 0; // Convert string to number, default to 0 if invalid
        return acc + rate;
      }, 0),
      tot_cgst: invoicePostData.reduce((acc, item) => {
        const rate = item.item_cgst ? Number(item.item_cgst) : 0; // Convert string to number, default to 0 if invalid
        return acc + rate;
      }, 0),
      tot_sgst: invoicePostData.reduce((acc, item) => {
        const rate = item.item_sgst ? Number(item.item_sgst) : 0; // Convert string to number, default to 0 if invalid
        return acc + rate;
      }, 0),
      tot_igst: 0,
      tot_round: form.getValues("roundOff"),
      tot_discount: form.getValues("discount"),
      year_id: finId,
  ...
  const getPurchaseVoucherApiCall = async (
    ...
    try {
      ...
      if (res.status === 200) {
        dispatch(getPurchaseVoucherData(res.data.details));
