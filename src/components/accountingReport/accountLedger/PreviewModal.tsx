...
import { PreviewModalProps } from "@/types/accountingReport/AccountLedgerTypes";
...
const PreviewModal: FC<PreviewModalProps> = ({
  ...
  const pages = useMemo(
    () => chunkRows(accountLedgerData, PRINT_ROWS_PER_PAGE),
    [accountLedgerData],
  );

  const debitTotal = accountLedgerData?.reduce((sum, entry) => {
    return sum + (parseFloat(entry.Debit) || 0);
  }, 0);

  const creditTotal = accountLedgerData?.reduce((sum, entry) => {
    return sum + (parseFloat(entry.Credit) || 0);
  }, 0);

  const ledgerName = legderData?.find((data) => data.Id.toString() === ledgerId)
    ?.Ledger_Name;
        ...
        return (
          ...
          >
            <PrintOrgHeader
              ...
              title={`Account Ledger - ${ledgerName || ""} From ${fromDate} To ${toDate}`}
              compact={pageIndex > 0}
            />
              ...
              <table className="w-full border-collapse overflow-hidden rounded-xl border border-black/[0.08]">
                <thead className={printTheadClass}>
                  <tr>
                    ...
                    <th className={printThClass}>Vouch No.</th>
                ...
                <tbody>
                  ...
                  {isLastPage && accountLedgerData.length > 0 ? (
