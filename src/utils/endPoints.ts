...
function appendPerPage(url: string, perPage?: number) {
  if (perPage == null || perPage <= 0) return url;
  return `${url}${url.includes("?") ? "&" : "?"}per_page=${perPage}`;
}

function appendDateRange(url: string, fromDate?: string, toDate?: string) {
  let next = url;
  if (fromDate) {
...
export const endPoints = {
  ...
  getItemModel: (orgId: number | string, page: number, perPage?: number) =>
    appendPerPage(
  ...
  getItemSize: (orgId: number | string, page: number, perPage?: number) =>
    appendPerPage(
  ...
  getOrderBooking: (
    ...
    const base = `${createApi}Org/ProcessInventory/GetActiveOrder?org_id=${orgId}&page=${page}&keyword=${keyword}`;
    if (perPage == null || perPage <= 0) return base;
    // Send both common param names so backend page-size is applied reliably.
    return `${base}&per_page=${perPage}&limit=${perPage}`;
  },
  ...
  getGstBill: (
    ...
    appendPerPage(
      `${createApi}Org/ProcessInventory/GetGstBill?org_id=${orgId}&page=${page}&keyword=${keyword}`,
      perPage
    ),
  deleteGstBill: `${createApi}Org/ProcessInventory/CancelGstBill`,
  getGstBillPrint: (orgId: number | string, salesId: string | number) =>
    `${createApi}Org/ProcessInventory/GetGstBillPrint?org_id=${orgId}&sales_id=${salesId}`,
